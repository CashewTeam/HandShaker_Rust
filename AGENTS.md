...
1. 先阅读任务涉及的文档和模块，使用 `rg` 搜索现有实现与测试。
2. 检查工作树，保留用户已有修改，不覆盖无关变更。
3. 对协议任务先写清已经确认的线路事实和待验证假设。
4. 做最小、直接的根因修复；不要顺手扩展未要求的功能。
5. 使用 `apply_patch` 修改文件；复制或移动文件时使用系统命令，不手工重新输出文件内容。
6. 为行为变更补充对应层级的单元、状态机、集成或 CLI 测试。
7. 更新受影响的协议文档、README、语言资源和版本。
8. 完成验证并检查最终 diff，确认没有生成物、敏感信息或残留 forward。
