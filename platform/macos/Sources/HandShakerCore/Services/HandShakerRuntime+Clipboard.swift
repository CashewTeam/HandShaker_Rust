...
extension HandShakerRuntime {
    ...
    public func clipboardList(sessionID: UInt64) throws -> [ClipboardEntry] {
    ...
    /// `{"set":true}`).
    public func clipboardSet(sessionID: UInt64, text: String) throws {
    ...
    /// result `{"deleted":true}`).
    public func clipboardDelete(sessionID: UInt64, timestampMs: Int64) throws {
    ...
    /// `{"cleared":true}`).
    public func clipboardClear(sessionID: UInt64) throws {
        try handle.withRuntime { runtime in
