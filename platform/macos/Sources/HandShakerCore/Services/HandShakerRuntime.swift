...
///
/// Threading model:
///  - The actor serializes all FFI calls; methods are synchronous `throws`
///    inside the actor (short calls block the calling thread on the
///    runtime's executor — never call them from a UI main thread).
...
public actor HandShakerRuntime {
    ...
    public func shutdown() async {
        shutdownFlag.set()
        _ = try? handle.withRuntime { runtime in
    ...
    public func listDevices() throws -> [DeviceDescriptor] {
    ...
    /// empty-array lie.
    public func discoverDevices() throws -> DeviceDiscoveryResult {
        try handle.withRuntime { runtime in
    ...
    public func connect(_ request: ConnectRequest) throws -> SessionSnapshot {
        let body = try ServicesJSON.encode(request)
        let sessionID = try handle.withRuntime { runtime in
        ...
        return try session(sessionID: sessionID.sessionID)
    }

    /// Close the session (`hs_disconnect`, result `{"disconnected":true}`).
    public func disconnect(sessionID: UInt64) throws {
        try handle.withRuntime { runtime in
    ...
    public func session(sessionID: UInt64) throws -> SessionSnapshot {
        try handle.withRuntime { runtime in
    ...
    /// `{"round_trip_ms":N}`).
    public func ping(sessionID: UInt64) throws -> PingResult {
        try handle.withRuntime { runtime in
    ...
    public func diagnostics() throws -> RuntimeDiagnostics {
        try handle.withRuntime { runtime in
