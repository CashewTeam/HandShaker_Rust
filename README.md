...
- **M8.1 Phase C 传输事件与取消（已完成）**：进度事件携带 `total_bytes`
  并按 100ms/256KiB 节流发布（30MB 真机实测 108 个事件）；取消立即终态
  （`finished_at_ms` + 事件，不等待后台任务）；本地/手机端取消经
  `TransferCancelled`/`RemoteCancelled` 区分；Transfer history 有容量
  （默认 64）/TTL 边界；Core typed events 已桥接到 Application
  （`ConnectionLost`、`ClipboardChanged`/`MediaChanged`/`RemoteFileChanged`
  携带 DTO payload，未知事件安全 Warning）；请求级连接丢失检测
  （Session `Failed` + 事件 + 后续请求 `SessionClosed` 快速失败）；
...
- **Application API v1 正式冻结（1.0.0，2026-08-04）**：审计 DoD 16/16
  ...
  major 递增。遗留：`RuntimeStarted`/`DeviceAdded`/`DeviceRemoved`
  事件变体属契约但运行时暂不发布（无 discovery watcher）。
