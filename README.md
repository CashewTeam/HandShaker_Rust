...
- 已在真实设备（Smartisan OD103 / Android 7.1.1）上完成**抓包验证**（`docs/14-capture-validation.md`）：
  ...
  - 验证工具见 `tools/capture/`。
- Rust CLI `0.1.1` 首版已进入可运行阶段：当前实现 ADB 接入、设备信息、文件管理、单文件上传下载、剪贴板和常驻 shell。
...
默认输出中文，所有用户可见文案从独立语言文件加载；自动化调用可使用 `--output json`，传输事件流可使用 `--output jsonl`。危险操作和覆盖在非交互环境中需要显式添加 `--yes`。
